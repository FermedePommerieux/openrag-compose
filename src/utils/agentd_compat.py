...
``name`` field, while AgentD constructs it without one.  The resulting
validation error interrupts the response stream after retrieval has already
completed.

The function below adapts only that event factory.  It preserves a name when
the caller supplies one and otherwise uses an empty value: the authoritative
tool name is already carried by the surrounding ``response.output_item``
events.  The guard is schema-aware and idempotent, so it becomes a no-op once
...
def ensure_agentd_openai_event_compatibility() -> bool:
    ...
    Returns ``True`` only when the adapter was installed.  Keeping the check
    at the dependency boundary avoids changing application stream semantics
    and makes the workaround self-removing after a compatible AgentD upgrade.
    """
