...
class AgentConfig:
    ...
    # Query planning is an application-runtime concern too. Empty values mean
    # "follow the agent selection" for backward compatibility; once either is
    # set through /api/settings it can evolve independently without an image,
    # Kubernetes, or GitOps change.
    planner_model: str = ""
    ...
    _v060_system_prompt: ClassVar[str] = system_prompt
    system_prompt: str = field(  # type: ignore[no-redef]
...
class ConfigManager:
    ...
    def _load_env_overrides(
        ...
        # Agent/planner model selection and the system prompt deliberately do
        # not have environment overrides. They are persisted OpenRAG runtime
        # settings. Provider credentials and endpoints above remain valid
        # infrastructure inputs, but deployment configuration must not select
