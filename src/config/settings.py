...
# adapter once during configuration import, before any client is patched.
...
def get_langflow_docling_url() -> str:
    """Return the Docling URL from Langflow's network perspective."""
    return os.getenv("DOCLING_SERVE_URL", "http://host.docker.internal:5001").rstrip("/")


def get_langflow_opensearch_url() -> str:
    """Return the OpenSearch URL from Langflow's network perspective."""
...
class AppClients:
    ...
    async def _create_langflow_global_variable(
        ...
        """Create a Langflow global variable with the requested type."""
        payload = {
            "name": name,
            "value": value,
            "default_fields": [],
    ...
    async def _update_langflow_global_variable(
        ...
        """Update a Langflow global variable, recreating it to change type."""
        try:
