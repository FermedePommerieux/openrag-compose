...
async def test_migrate_unversioned_retrieval_v2_flow_synchronized_by_gitops():
    ...
    assert transport.flow["data"]["openrag_retrieval_version"] == 11
    agent_node = next(
        node
...
def test_version_6_migration_preserves_settings_managed_model_values():
    ...
    assert migrated["data"]["openrag_retrieval_version"] == 11
...
@pytest.mark.asyncio
async def test_migrate_exact_phase2_version_10_to_deterministic_guard_v11():
    service = FlowsService()
    transport = _RetrievalMigrationTransport()
