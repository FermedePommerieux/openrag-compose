...
def _previous_rebuilt_retrieval_v15_flow() -> dict:
    """Load v15 after model rebuild but before the inspected annotation fix."""
    raw = subprocess.check_output(
        [
            "git",
            "show",
            "84bc5e9ca40a1627b4d46ebb34ff5f8ebe67d590:flows/openrag_agent.json",
        ],
        cwd=ROOT,
        text=True,
    )
...
def _previous_opt_in_exhaustive_retrieval_v15_flow() -> dict:
    """Load the deployed v15 graph before exhaustive search became mandatory."""
    raw = subprocess.check_output(
        [
            "git",
            "show",
            "e42c4feb1ed37134d567a2398ec301acd08c00f4:flows/openrag_agent.json",
        ],
        cwd=ROOT,
        text=True,
    )
...
async def test_migrate_unversioned_retrieval_v2_flow_synchronized_by_gitops():
    ...
    assert transport.flow["data"]["openrag_retrieval_version"] == 19
    agent_node = next(
        node
...
def test_migrated_graph_accepts_runtime_managed_prompt_but_rejects_code_edits():
    ...
    flow["data"]["openrag_retrieval_version"] = 19
    language_model = next(
        node
        for node in flow["data"]["nodes"]
...
async def test_migrate_exact_fail_closed_version_13_to_audit_closeout_version_14():
    ...
    assert (
        "_uses_canonical_coverage_certificate" in agent["data"]["node"]["template"]["code"]["value"]
    )
    assert (
        "_without_opaque_relation_targets" in retrieval["data"]["node"]["template"]["code"]["value"]
    )
...
async def test_migrate_exact_version_15_dynamic_schema_repair():
    ...
    assert (
        ...
        not in (retrieval["data"]["node"]["template"]["code"]["value"])
    )


@pytest.mark.asyncio
async def test_migrate_exact_version_15_dynamic_function_annotation_repair():
    service = FlowsService()
    transport = _RetrievalMigrationTransport()
    transport.flow = _previous_rebuilt_retrieval_v15_flow()
    ...
    assert (
        ...
        in (retrieval["data"]["node"]["template"]["code"]["value"])
    )
...
async def test_migrate_exact_version_15_no_metadata_q1_fallback():
    ...
    code = retrieval["data"]["node"]["template"]["code"]["value"]
    assert 'evidence_mode="scope_exhaustive"' in code


@pytest.mark.asyncio
async def test_migrate_exact_version_15_to_mandatory_exhaustive_version_16():
    service = FlowsService()
    transport = _RetrievalMigrationTransport()
    transport.flow = _previous_opt_in_exhaustive_retrieval_v15_flow()
...
@pytest.mark.asyncio
async def test_migrate_recognized_v18_native_toolkit_preserves_prompt(repaired):
    service = FlowsService()
    transport = _RetrievalMigrationTransport()
    if repaired:
        flow = service._load_retrieval_v2_template()
    else:
        flow = json.loads(
            subprocess.check_output(
                [
                    "git",
                    "show",
                    "7cd6e1e4a0ae7ab662a419454e4509f19a485b53:flows/openrag_agent.json",
                ],
                cwd=ROOT,
                text=True,
            )
        )
    flow["data"]["openrag_retrieval_version"] = 18
    flow["locked"] = True
    agent = next(
        n["data"]["node"]
        for n in flow["data"]["nodes"]
        if n["data"]["node"].get("display_name") == "Agent"
    )
    agent["template"]["system_prompt"]["value"] = "operator managed prompt"
    assert service._is_known_previous_retrieval_v2_flow(flow)
    transport.flow = flow
    with (
        patch("services.flows_service.clients.langflow_request", side_effect=transport.__call__),
        patch.object(
        ...
        result = await service.migrate_persisted_retrieval_flow()
        again = await service.migrate_persisted_retrieval_flow()
    assert result["status"] == "migrated"
    ...
    agent = next(
        n["data"]["node"]
        for n in transport.flow["data"]["nodes"]
        if n["data"]["node"].get("display_name") == "Agent"
    )
    assert agent["template"]["system_prompt"]["value"] == "operator managed prompt"
    retrieval = next(
        n["data"]["node"]
        for n in flow["data"]["nodes"]
        if n["data"]["node"].get("display_name") == "OpenRAG Retrieval v2"
    )
    retrieval["template"]["code"]["value"] += "\n# unrecognized operator code"
    assert not service._is_known_previous_retrieval_v2_flow(flow)
