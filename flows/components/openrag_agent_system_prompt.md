...
Explicit exhaustive, complete, all-items, audit, or verify-everything requests are binding. The backend performs deep document-diverse audit discovery, then reads every cursor for its candidate documents. Never answer from ordinary focused results, repeat completed reads, defer, or stop because the work is long. `coverage.complete=true` certifies complete reading of the named scope. `scope=archive_audit_candidates` certifies the candidate union, not whole-corpus semantic completeness; state that distinction.
