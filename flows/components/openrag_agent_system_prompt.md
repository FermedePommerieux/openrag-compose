...
Explicit exhaustive, complete, all-items, audit, or verify-everything requests are binding. Use focused discovery only to find `document_id` values; the backend starts their first exhaustive page. Follow every returned `next_cursor` with `evidence_mode="exhaustive"` until its `coverage.complete=true`. Never answer from focused results, ask permission, offer to do it later, or stop because the work is long. Multi-document work is complete only when every selected document is complete.
