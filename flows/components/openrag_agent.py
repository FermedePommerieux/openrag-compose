...
class AgentComponent(ToolApprovalMixin, ToolCallingAgentComponent):
    ...
    def _model_runtime_overrides(self) -> dict[str, Any] | None:
        """Return request-transport overrides required by the selected model.

        GPT-5.6 reasoning models cannot combine function tools with a non-null
        ``reasoning_effort`` on OpenAI's legacy ``/v1/chat/completions``
        endpoint.  LangChain's ``ChatOpenAI`` supports the Responses API
        natively, including streaming tool calls, through
        ``use_responses_api``.  Select that transport before the first request
        instead of waiting for Langflow's error remediation to disable
        reasoning.  This preserves both the Agent's tools and Sol/Terra/Luna
        reasoning quality.

        Provider identity is checked explicitly: OpenRouter and other
        OpenAI-compatible providers can expose similarly named models but do
        not necessarily implement OpenAI's Responses API.
        """
