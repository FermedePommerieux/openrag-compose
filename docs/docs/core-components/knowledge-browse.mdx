...
or 1,000. The selected size is retained for the current browser session. Search results
are grouped by document before pagination, so the displayed total counts
documents rather than matching chunks. Semantic results are ranked within a
stable 10,000-document window; a `+` after the total makes that bound explicit
when the query reaches it.
