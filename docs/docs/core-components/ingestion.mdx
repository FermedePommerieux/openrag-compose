...
OpenRAG has verified (tested) support for the following file types:
...
You can attempt to ingest any other file type supported by Docling, but the ingestion might fail or produce suboptimal results.
You can submit an [OpenRAG GitHub issue](https://github.com/langflow-ai/openrag/issues) to request support for specific file types or report ingestion errors.
...
and use a separately managed custom flow when you need a different execution
graph. This does not make direct edits to the locked managed Agent flow a
