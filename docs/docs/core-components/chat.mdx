...
owner-scoped audit job. Closing the page or losing the streaming connection
does not cancel the producer. When a live page loses its stream, the chat polls
the existing audit identifier and displays its stored result when complete.
