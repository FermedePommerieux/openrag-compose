...
## Retrieval progress and usage

Streaming chat displays small factual progress updates while the normal
OpenSearch retrieval is running: direct lane search, rank convergence, PROV-O
relation traversal and document-graph construction. These events contain only
phase codes and integer counters. They are observability, not evidence, and
never enter the answer or conversation history.

Assistant messages display the input, cached, output and reasoning token usage
reported by the response provider, plus an estimated USD cost when that model
has a known price. OpenSearch retrieval and PROV-O traversal are local and do
not consume LLM tokens. There is no detached archive-audit job or hidden
multi-model validation bill.
