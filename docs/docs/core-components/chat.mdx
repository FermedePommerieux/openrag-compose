...
One or more specialized tools can be attached to the **Tools** port to extend the agent's capabilities. In this case, there are two tools: **MCP Tools** and the thin **OpenRAG Retrieval v2** tool. The retrieval path is Chat → Langflow Agent → OpenRAG Retrieval v2 → backend SearchService/RetrievalService → OpenSearch.
...
* **OpenRAG Retrieval v2** component: Connected to the **Agent** component's **Tools** port, this thin tool calls the backend `/search` endpoint with the end-user identity and active search context. The backend, not the flow, owns BM25/vector retrieval, RRF, filters, provenance, and user-scoped OpenSearch DLS. See [Retrieval v2](/retrieval-v2).
...
## Exhaustive audit continuity and usage

An explicitly exhaustive archive request is executed as a durable,
owner-scoped audit job. Losing a live page's streaming connection does not
cancel the producer: that page polls the existing audit identifier and
displays its stored result when complete. The durable status also preserves a
non-empty terminal explanation, token usage and progress counters when an
upstream operation fails; the UI must show that failure rather than remaining
on `Thinking`.

Assistant messages display provider-reported input, cached, output and
reasoning tokens. They also display a USD estimate based on the dated public
price table shipped with OpenRAG. For exhaustive audits the figures aggregate
the complete operation, including internal reasoning workers and query
embeddings; they are not limited to the final synthesis call. An unavailable
price is shown as unavailable rather than estimated.
