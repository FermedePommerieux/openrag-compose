...
| `INGESTION_TIMEOUT` | `3600` | Document ingestion timeout limit in seconds for each file, including direct Docling polling. Increase this value if you ingest OCR-heavy or very large documents. Must be greater than or equal to [`LANGFLOW_TIMEOUT`](#langflow-settings).  |
