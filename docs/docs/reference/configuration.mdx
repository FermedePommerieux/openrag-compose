...
| `OPENRAG_AUDIT_REASONING_MODEL` | Agent model | Optional lower-cost model used for high-volume archive candidate classification and hierarchical evidence workers. The final chat response continues to use the selected agent model. |
...
| `LANGFLOW_STREAM_TIMEOUT` | `21600` | Read timeout for the OpenAI-compatible Langflow response stream. It wraps complete tool execution and therefore permits a detached exhaustive audit to run for up to six hours instead of inheriting the OpenAI SDK's 600-second default. |
