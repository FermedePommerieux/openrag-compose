...
You can [inspect this ingestion flow in Langflow](/agents#inspect-and-modify-flows).
For graph-level experimentation, use a separately managed custom flow; direct
changes to the locked managed Agent flow are not a supported configuration
path.
...
* [**Split Text** component](https://docs.langflow.org/split-text): Splits the processed text into chunks, based on the configured [chunk size and overlap settings](/ingestion-configure).
...
* [**Embedding Model** component](https://docs.langflow.org/components-embedding-models): Generates vector embeddings using your selected [embedding model](/ingestion-configure#set-the-embedding-model-and-dimensions).
* [**OpenSearch** component](https://docs.langflow.org/bundles-elastic#opensearch): Stores the processed documents and their embeddings in your OpenRAG [OpenSearch knowledge base](/knowledge).

   The default address for the OpenSearch knowledge base is `https://localhost:9200`. To change this address, edit the `OPENSEARCH_PORT` [environment variable](/reference/configuration#opensearch-settings).
